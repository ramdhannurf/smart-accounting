  ...
  await i18nInstance
    ...
    .use(
      resourcesToBackend(
        ...
        (lang: string, ns: string) => import(`../translations/${lang}/${ns}.json`),
      ),
    )
...
export async function createTranslationFunction(ns: string) {
...
export function getLocale() {
  return (Cookies.get(LANGUAGE_COOKIE) ?? FALLBACK_LOCALE) as Locales;
}
